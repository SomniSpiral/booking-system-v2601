...
class BookingCatalog {
    constructor(config = {}) {
        ...
        this.config = {
            ...
            filterDropdownMenuId: config.filterDropdownMenuId || 'filterDropdownMenu',
    ...
    cacheDomElements() {
        this.elements = {
            loadingIndicator: document.getElementById(this.config.loadingIndicatorId),
            catalogItemsContainer: document.getElementById(this.config.containerId),
            categoryFilterList: document.getElementById(this.config.categoryFilterId),
            pagination: document.getElementById(this.config.paginationId),
            requisitionBadge: document.getElementById(this.config.requisitionBadgeId),
            ...
            clearSearchBtn: document.getElementById(this.config.clearSearchBtnId),
            filterDropdown: document.getElementById(this.config.filterDropdownId)
        };
    }
    ...
    setInitialUI() {
        ...
        const statusRadio = document.querySelector(`#${this.config.filterDropdownMenuId} .status-option[data-status="${this.statusFilter}"]`);
        if (statusRadio) {
            statusRadio.checked = true;
        }

        // Set initial layout radio
        const layoutRadio = document.querySelector(`#${this.config.filterDropdownMenuId} .layout-option[data-layout="${this.currentLayout}"]`);
        if (layoutRadio) {
    ...
    setupEventListeners() {
        ...
        this.elements.catalogItemsContainer.addEventListener('click', async (e) => {
        ...
        document.querySelectorAll(`#${this.config.filterDropdownMenuId} .status-option`).forEach((radio) => {
        ...
        document.querySelectorAll(`#${this.config.filterDropdownMenuId} .layout-option`).forEach((radio) => {
            radio.addEventListener('change', () => {
                if (radio.checked) {
    ...
    validateQuantityInput(input) {
        const button = input.closest('.equipment-quantity-selector').querySelector('.add-remove-btn');
        const id = button.dataset.id;
        const quantity = parseInt(input.value) || 0;

        const equipmentItem = this.allItems.find(item =>
            item.equipment_id === parseInt(id) && this.catalogType === 'equipment'
        );
        ...
        if (equipmentItem) {
            ...
            if (quantity > availableQty) {
                ...
                let errorMsg = input.parentNode.querySelector('.quantity-error');
                if (!errorMsg) {
                ...
                const errorMsg = input.parentNode.querySelector('.quantity-error');
    ...
    /**
     * Switch catalog type (venues, rooms, equipment)
     */
    ...
    /**
     * Load catalog data from API or cache
     */
    async loadCatalogData() {
        try {
            ...
            if (this.searchQuery && this.searchQuery.trim() !== '') {
                itemsUrl += `?search=${encodeURIComponent(this.searchQuery)}`;
            }

            const cached = !this.searchQuery ? this.getFromCache(this.catalogType) : null;
            ...
            if (cached) {
                console.log(`Loading ${this.catalogType} from localStorage cache`);
                this.allItems = cached.items || [];
                this.itemCategories = cached.categories || [];

                this.allItems = this.allItems.filter(item => this.allowedStatusIds.includes(item.status?.status_id));
                this.applyCatalogTypeFilter();
                ...
                console.log(`Fetching ${this.catalogType} from API with search: ${this.searchQuery}`);

                [itemsData, categoriesData] = await Promise.all([
                    this.fetchData(itemsUrl),
                    this.fetchData(api.categories)
                ]);

                this.allItems = this.extractItems(itemsData);
            ...
            this.showToast(`Failed to load ${this.catalogType}. Please try again.`, "error");
            if (this.elements.loadingIndicator) {
                this.elements.loadingIndicator.style.display = "none";
            }
    ...
    applyCatalogTypeFilter() {
        if (this.catalogType === 'venues') {
            this.allItems = this.allItems.filter(item => !item.parent_facility_id);
        } else if (this.catalogType === 'rooms') {
            this.allItems = this.allItems.filter(item => item.parent_facility_id !== null);
        }
    }
    ...
    /**
     * Fetch selected items from requisition form
     */
    ...
    updateCartBadge() {
        ...
        if (this.selectedItems.length > 0) {
            this.elements.requisitionBadge.textContent = this.selectedItems.length;
    ...
    filterItems() {
                ...
                if (this.catalogType === 'equipment') {
                    if (item.items && Array.isArray(item.items)) {
                        return item.items.some(equipmentItem =>
                            equipmentItem.item_name && equipmentItem.item_name.toLowerCase().includes(query)
                        );
                    }
                    return false;
                } else {
                    const nameMatch = item.facility_name && item.facility_name.toLowerCase().includes(query);
        ...
        if (this.statusFilter === "Available") {
            this.filteredItems = this.filteredItems.filter(item => item.status.status_id === 1);
        } else if (this.statusFilter === "Unavailable") {
            this.filteredItems = this.filteredItems.filter(item => item.status.status_id === 2);
        }
        ...
        if (!allCategoriesCheckbox.checked) {
            if (this.catalogType === 'equipment') {
                ...
                this.filterFacilitiesByCategory(categoryCheckboxes, subcategoryCheckboxes);
            }
        }

        // Additional filter for data integrity
        if (this.catalogType === 'venues') {
            this.filteredItems = this.filteredItems.filter(item => !item.parent_facility_id);
        } else if (this.catalogType === 'rooms') {
            this.filteredItems = this.filteredItems.filter(item => item.parent_facility_id !== null);
        }
    }
    ...
    filterAndRenderItems() {
        ...
        this.renderPagination(this.filteredItems.length);
    }
    ...
    renderItems(items) {
        ...
        const paginatedItems = items.slice(startIndex, startIndex + this.config.itemsPerPage);

        this.elements.catalogItemsContainer.innerHTML = "";

        if (paginatedItems.length === 0) {
            this.elements.catalogItemsContainer.classList.remove("grid-layout", "list-layout");
        ...
        this.elements.catalogItemsContainer.classList.remove("grid-layout", "list-layout");
        this.elements.catalogItemsContainer.classList.add(`${this.currentLayout}-layout`);

        if (this.currentLayout === "grid") {
            this.renderGridLayout(paginatedItems);
        } else {
            this.renderListLayout(paginatedItems);
        }
    ...
    renderGridLayout(items) {
        this.elements.catalogItemsContainer.innerHTML = items.map(item => {
            ...
            const description = this.truncateText(item.description || 'No description available', 100);
    ...
    renderListLayout(items) {
        this.elements.catalogItemsContainer.innerHTML = items.map(item => {
            ...
            const description = this.truncateText(item.description || 'No description available', 150);
    ...
    getEquipmentActionsHtml(item) {
        ...
        const selectedItem = isSelected ? this.selectedItems.find(
            selectedItem => selectedItem.type === 'equipment' &&
                parseInt(selectedItem.equipment_id) === item.equipment_id
        ) : null;
    ...
    async removeFromForm(id, type) {
        try {
            ...
            this.selectedItems = response.data.selected_items || [];
            this.showToast(`${type.charAt(0).toUpperCase() + type.slice(1)} removed from form`);
            await this.updateAllUI();
            localStorage.setItem('formUpdated', Date.now().toString());
    ...
    showItemDetails(itemId) {
        ...
        const primaryImage = this.getPrimaryImage(item);
        const isUnavailable = item.status.status_id === 2;
        const itemType = this.catalogType === 'equipment' ? 'equipment' : 'facility';
        const isSelected = this.selectedItems.some(
            selectedItem => selectedItem.type === itemType &&
                parseInt(selectedItem[`${itemType}_id`]) == itemId
        );

        document.getElementById("itemDetailModalLabel").textContent = isEquipment ? item.equipment_name : item.facility_name;
    ...
    showFacilityAvailability(button) {
        try {
            ...
            if (modalTitleElement) {
                ...
                let title = isEquipment ? 'Equipment Availability' : 'Facility Availability';
            ...
            if (categoryElement) categoryElement.textContent = itemData.category || 'N/A';
            ...
            if (modalElement) {
                ...
                modalElement.addEventListener('hidden.bs.modal', () => {
                    if (this.availabilityCalendarInstance && this.availabilityCalendarInstance.calendar) {
                        try {
    ...
    async initEquipmentAvailabilityCalendar(equipmentId) {
        ...
        try {
            ...
            const originalHideLoadingOverlay = this.availabilityCalendarInstance.hideLoadingOverlay;
            this.availabilityCalendarInstance.hideLoadingOverlay = () => {
                ...
                if (availabilityOverlay) availabilityOverlay.classList.add('hidden');
                if (typeof originalHideLoadingOverlay === 'function') originalHideLoadingOverlay.call(this);
            };

            this.availabilityCalendarInstance.updateLoadingState = (isLoading) => {
                const availabilityOverlay = document.getElementById('availabilityLoadingOverlay');
                if (availabilityOverlay) {
                    if (isLoading) availabilityOverlay.classList.remove('hidden');
            ...
            const originalLoadEvents = this.availabilityCalendarInstance.loadCalendarEvents;
            this.availabilityCalendarInstance.loadCalendarEvents = async () => {
                try {
                    const headers = {};
                    if (this.availabilityCalendarInstance.config.isAdmin && this.availabilityCalendarInstance.config.adminToken) {
                        headers["Authorization"] = `Bearer ${this.availabilityCalendarInstance.config.adminToken}`;
                    }

                    const response = await fetch(`${this.availabilityCalendarInstance.config.apiEndpoint}`, { headers });
                    const result = await response.json();

                    if (result.success && result.data) {
                        this.availabilityCalendarInstance.allEvents = result.data
                            .filter(event => event != null)
                            .filter(event => {
                                const eventEquipment = event.extendedProps?.equipment || [];
                                return eventEquipment.some(eq => eq.equipment_id && eq.equipment_id.toString() === equipmentId.toString());
                            })
                            .map(event => {
                                const statusName = event.extendedProps?.status;
                                const statusColor = this.availabilityCalendarInstance.statusColors[statusName] || event.extendedProps?.color || "#007bff";
                                return { ...event, color: statusColor, extendedProps: { ...event.extendedProps, color: statusColor } };
                            });

                        this.availabilityCalendarInstance.applyFilters();

                        if (this.availabilityCalendarInstance.allEvents.length === 0) {
                    ...
                    if (result.success && result.data) {
                        this.availabilityCalendarInstance.allEvents = result.data
                            ...
                            .filter(event => {
                                ...
                                return eventFacilities.some(f => f.facility_id && f.facility_id.toString() === facilityId.toString());
                            })
                            .map(event => {
                                const statusName = event.extendedProps?.status;
                                const statusColor = this.availabilityCalendarInstance.statusColors[statusName] || event.extendedProps?.color || "#007bff";
