...
class BookingCatalog {
    constructor(config = {}) {
        this.config = {
            ...
            apiEndpoints: config.apiEndpoints || {
                venues: { items: "/api/facilities/venues", categories: "/api/facility-categories/venues" },
                rooms: { items: "/api/facilities/rooms", categories: "/api/facility-categories/rooms" },
                equipment: { items: "/api/equipment", categories: "/api/equipment-categories" },
            },
    ...
    setInitialUI() {
        document.querySelectorAll(".catalog-type-tab").forEach(tab => {
            if (tab.dataset.type === this.catalogType) tab.classList.add("active");
        });

        const statusRadio = document.querySelector(`#${this.config.filterDropdownMenuId} .status-option[data-status="All"]`);
        if (statusRadio) statusRadio.checked = true;

        const layoutRadio = document.querySelector(`#${this.config.filterDropdownMenuId} .layout-option[data-layout="grid"]`);
    ...
    setupEventListeners() {
        ...
        if (this.elements.searchForm) {
            this.elements.searchForm.addEventListener("submit", (e) => e.preventDefault());
        }
        ...
        this.elements.catalogItemsContainer.addEventListener("click", async (e) => {
            ...
            if (button && !button.disabled) await this.handleAddRemoveAction(button);
        });
    ...
    async switchCatalogType(type) {
        ...
        if (this.elements.catalogHeroTitle) this.elements.catalogHeroTitle.textContent = titles[type];
        ...
        if (buildingCard) buildingCard.style.display = type === "rooms" ? "block" : "none";
    ...
    async loadCategories() {
        ...
        this.itemCategories = Array.isArray(response) ? response : (response.data || []);
    ...
    async loadBuildings() {
        const buildingFilterList = document.getElementById("buildingFilterList");
        if (!buildingFilterList) return;
        
        const response = await this.fetchData("/api/facilities/parent-buildings-for-rooms");
        const buildings = response.data || [];
        
        buildingFilterList.innerHTML = buildings.length === 0 
    ...
    renderCategoryFilters() {
        ...
        if (this.catalogType === "rooms") {
            categories = this.itemCategories.filter(c => c.category_id === 2 || c.category_id === 3);
        }
        ...
        categories.forEach(category => {
            ...
            const toggleArrow = container.querySelector(`[data-category-id="${category.category_id}"] .toggle-arrow`);
            if (toggleArrow) {
                const subcatList = container.querySelector(`[data-category-id="${category.category_id}"] .subcategory-list`);
                toggleArrow.addEventListener("click", () => {
                    const isExpanded = subcatList.style.maxHeight !== "0px";
                    subcatList.style.maxHeight = isExpanded ? "0" : `${subcatList.scrollHeight}px`;
                    toggleArrow.classList.replace(isExpanded ? "bi-chevron-up" : "bi-chevron-down",
    ...
    setupCategoryEvents() {
        ...
        const categoryCheckboxes = document.querySelectorAll(".category-filter:not(#allCategories)");
        ...
        subcategoryCheckboxes.forEach(sub => {
            sub.addEventListener("change", () => {
                ...
                const parentCategory = [...categoryCheckboxes].find(cb => cb.value === catId);
                const siblings = [...subcategoryCheckboxes].filter(s => s.dataset.category === catId);
                const allSiblingsChecked = siblings.length > 0 && siblings.every(s => s.checked);
    ...
    getSelectedFilters() {
        const categories = [...document.querySelectorAll(".category-filter:checked")]
            .filter(cb => cb.id !== "allCategories").map(cb => cb.value);
        const subcategories = [...document.querySelectorAll(".subcategory-filter:checked")].map(cb => cb.value);
        const buildings = [...document.querySelectorAll(".building-filter:checked")].map(cb => cb.value);
    ...
    renderItems() {
        ...
        const renderMethod = this.currentLayout === "grid" ? this.renderGridCard : this.renderListCard;
        container.innerHTML = this.allItems.map(item => renderMethod.call(this, item)).join("");
    ...
    renderGridCard(item) {
        ...
        const buildingInfo = this.catalogType === "rooms" && item.parent_facility ? `<span><i class="bi bi-building"></i> ${item.parent_facility.facility_name}</span>` : "";
    ...
    getEquipmentActions(item) {
        const isSelected = this.selectedItems.some(s => s.type === "equipment" && parseInt(s.equipment_id) === item.equipment_id);
        const selectedItem = this.selectedItems.find(s => s.type === "equipment" && parseInt(s.equipment_id) === item.equipment_id);
    ...
    getFacilityActions(item) {
        ...
        const isSelected = this.selectedItems.some(s => s.type === "facility" && parseInt(s.facility_id) === item.facility_id);
        
        if (isUnavailable) return `<button class="btn btn-secondary add-remove-btn form-action-btn" disabled>Unavailable</button>`;
        if (isSelected) return `<button class="btn btn-danger add-remove-btn form-action-btn" data-id="${item.facility_id}" data-type="facility" data-action="remove">Remove from form</button>`;
    ...
    renderPagination() {
        ...
        let html = "";
        if (current_page > 1) html += `<li class="page-item"><a class="page-link" href="#" data-page="${current_page - 1}">&laquo;</a></li>`;
        
        for (let i = Math.max(1, current_page - 2); i <= Math.min(last_page, current_page + 2); i++) {
            html += `<li class="page-item ${i === current_page ? "active" : ""}"><a class="page-link" href="#" data-page="${i}">${i}</a></li>`;
        }
        
        if (current_page < last_page) html += `<li class="page-item"><a class="page-link" href="#" data-page="${current_page + 1}">&raquo;</a></li>`;
        ...
        container.querySelectorAll(".page-link").forEach(link => {
            link.addEventListener("click", (e) => {
                ...
                this.loadCatalogData();
                window.scrollTo({ top: this.elements.catalogItemsContainer?.offsetTop - 100, behavior: "smooth" });
            });
        });
    }
    ...
    validateQuantityInput(input) {
        ...
        const equipmentItem = this.allItems.find(item => item.equipment_id === parseInt(id));
    ...
    async addToForm(id, type, quantity) {
        ...
        if (!response?.success) throw new Error(response?.message || "Failed to add item");
        this.showToast(`${type} added to form`, "success");
        if (this.config.onItemAdded) this.config.onItemAdded(id, type, quantity);
    }
    ...
    performSearch() {
        ...
        if (this.elements.clearSearchBtn) this.elements.clearSearchBtn.style.display = this.searchQuery ? "block" : "none";
        this.currentPage = 1;
        this.loadCatalogData();
    }
    ...
    showItemDetails(itemId) {
        const item = this.allItems.find(item => {
            if (this.catalogType === "equipment") return item.equipment_id == itemId;
        ...
        const isSelected = this.selectedItems.some(s => s.type === itemType && parseInt(s[`${itemType}_id`]) == itemId);
        ...
        document.getElementById("itemDetailContent").innerHTML = `
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="mt-3">
                        ${isUnavailable ? `<button class="btn btn-secondary" disabled>Unavailable</button>` : 
        ...
        const modalBtn = document.querySelector("#itemDetailContent .add-remove-btn");
        if (modalBtn) {
    ...
    showFacilityAvailability(button) {
        ...
        console.log("Show availability for:", button.dataset.itemName);
    }
...
if (typeof module !== "undefined" && module.exports) module.exports = BookingCatalog;